...
// Workflow board met Firestore (read + CRUD voor kaarten)
...
// Firestore setup (pakt automatisch DEV of MAIN o.b.v. hostname)
...
function buildToolbarAndForm() {
  ...
  // Formulier sectie (standaard verborgen)
...
function getNextSortValueForBacklog() {
  // Zoek hoogste sort in backlog kolom
  let maxSort = 0;
  state.cardsById.forEach(card => {
    if (card.columnId === state.backlogColumnId) {
  ...
  try {
    if (state.mode === "edit" && state.editingCardId) {
      ...
      if (!state.backlogColumnId) {
        alert("Geen Backlog kolom gevonden.");
        return;
      }
      const cardsRef = collection(db, COL_CARDS);
    ...
    // Opnieuw laden en formulier sluiten
    await reloadBoard();
    closeForm();
  } catch (err) {
    console.error("Fout bij opslaan kaart", err);
    alert("Er ging iets mis bij het opslaan van de kaart.");
  }
}
...
async function handleDeleteCard() {
  ...
  try {
    ...
    alert("Er ging iets mis bij het verwijderen van de kaart.");
  }
}
...
async function reloadBoard() {
  ...
  // Backlog kolom id zoeken (op basis van titel)
  const backlogCol = columns.find(c => c.title === "Backlog") || columns[0] || null;
