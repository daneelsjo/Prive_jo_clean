...
function handleActionError(action, error, context = {}, userMessage = "Er is een fout opgetreden.") {
  logEntry({ level: "error", page: "workflow", action, message: error.message, ...context });
      ...
      <div id="tab-content-timeline" class="wf-tab-content">
        ...
        <div class="wf-log-wrapper">
          ...
          <div class="wf-log-input-group">
            <input type="text" id="input-log-entry" placeholder="Typ notitie..." autocomplete="off">
        ...
        <div class="wf-links-wrapper">
          ...
          <div class="wf-link-input-group">
            <input type="text" id="input-link-name" placeholder="Naam">
            <input type="text" id="input-link-url" placeholder="URL">
            <button type="button" class="wf-btn wf-btn-primary wf-btn-small" id="btn-add-link">Toevoegen</button>
          </div>
          <div class="wf-link-list"></div>
        </div>
        <div class="wf-card-form-actions">
           <button type="button" class="wf-btn wf-btn-secondary btn-cancel-card" style="margin-left:auto;">Sluiten</button>
        </div>
      </div>
    </form>
  `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, user => {
    if (user) { state.uid = user.uid; getOrCreateBoard(user.uid).then(b => { state.boardId = b.id; reloadData(); }); }
    else setBoardMessage("Log in a.u.b.");
  });
});